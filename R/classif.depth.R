...
#,control=list(trace=FALSE,draw=TRUE)
 ...
 nvec<-table(group)
# p<-nvec[1]/n        
 if (depth %in% c("PD","HD","SD","RP","RPD","RT")){
...
#  if (CV!=TRUE){
   ...
   for (i in 1:ng) {
    ...
    if (depth %in% c("depth.HD","depth.PD","depth.MhD"))  par.depth$xx<-fdataobj[ind[,i],]   
...
#  }
  if (CV==TRUE) {
   ...
   for (j in 1:n) {
    ...
    for (i in 1:ng) {
     ...
     xnoji<-xnoj[ind[,i],]    
     nam<-c(paste("depth ",lev[i],sep=""),paste("depth ",paste(lev[-i],collapse=",")))
     if (depth %in% c("depth.HD","depth.PD","depth.MhD"))  par.depth$xx<-xnoji
 ...
 if (depth %in% c("HD","SD","PD","MhD"))  par.depth$x<-newfdataobj
