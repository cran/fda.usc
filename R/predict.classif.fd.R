...
if (C[ii]!='NULL()')     {
    ...
    while (C[ii]!='NULL()'&& ind.m<=len.metricc) {
     ...
     if (aa) { vv[ind.m]<-which(names(C)==names(formals(a1)[ind.m]))
           ii<-ii+1
           as[[ind.m]]<-C[[vv[ind.m]]]           }
...
if (length(new.fdata)!=dim(mdatos)[2]){stop("ERROR IN THE DATA DIMENSIONS")}
...
else {stop("ERROR IN THE DATA DIMENSIONS")}}
 if ((dim(mdatos)[1])!=(length(object$group)))
   {stop("ERROR IN THE DATA DIMENSIONS") }
group=object$group
if (is.factor(group)==FALSE) {group=as.factor(group)}
numg=nlevels(group)
...
pr=1;prob=0;vec=0
      as[[2]]<-new.fdataobj       #nndatos
      as[[1]]<-object$fdataobj       #mdatos
      mdist<-do.call(a1,as)
...
if (is.vector(new.fdata)) {
   ...
   if (is.matrix(new.fdata)) {
  if (dim(mdatos)[2]!=dim(new.fdata)[2]) {stop("ERROR IN THE DATA DIMENSIONS")}
  else {  numgrn=dim(new.fdata)[1]
          cat(numgrn,"New data are predicted\n")
          nndatos=new.fdata
          tr=3          }
      }
...
if ((dim(mdatos)[1])!=(length(object$group))){stop("ERROR IN THE DATA DIMENSIONS")}
as<-list()
C<-object$C
m<-object$m
imetric<-m[4]
if (imetric==0) {       a1<-metric.lp
      len.metricc<-length(formals(a1))
      vv<-array(0,dim=c(len.metricc))       }
else {    a1<-match.fun(C[[imetric]])
   len.metricc<-length(formals(a1))
   vv<-array(0,dim=c(len.metricc))  }
ii<-imetric+1
if (C[ii]!='NULL()')     {
...
mdist=array(0,dim=c(numgr,numgrn))
#### argvals
mm.fd=Data2fd(y=t(mm),argvals=tt,basisobj=object$basis.obj.opt)
pr=1;prob=0;vec=0
as[[2]]<- t(mm.fd$coefs) #si metric.lp deberían ser objetos fdata
as[[1]]<- t(object$coefs)    #si metric.lp deberían ser objetos fdata
mdist<-do.call(a1,as)                           ###
mat=array(0,dim=c(numgr,numg))
hmdist=mdist/object$h.opt
for (j in 1:numg) {
...
numgrn=dim(new.fdata)[1]
cat(numgrn,"new data are predicted \n")
nndatos=array(NA,dim=c(dim(new.fdata)[1],dim(new.fdata)[2]))
nndatos=new.fdata
tr=3
}}
...
pr=1;prob=0;vec=array(0,dim=c(numgrn))
      as[[2]]<-new.fdataobj       #nndatos
      as[[1]]<-object$fdataobj       #mdatos
      dist<-do.call(a1,as)
pgrup=array(0,dim=c(numg,numgrn))
for (k in 1:numgrn) {
    ...
    vec=quantile(dist[,k],prob=(object$knn.opt/(numgr)),type=4)
    l=which(dist[,k]<=vec)
    l2=which.min(dist[,k])
    for (j in 1:object$knn.opt)  pgrup[group[l[j]],k]=pgrup[group[l[j]],k]+1
    max2=which.max(pgrup[,k])
    if (length(max2)>1) {vvmax=as.character(group[l2])}
    else {vvmax=levels(group)[max2]}
    group.pred[k]=vvmax
    }
...
if (!probs.group) return(list(group.pred=as.factor(group.pred)))
      ...
      return(list(group.pred=as.factor(group.pred),prob.group=pgrup))}
}
